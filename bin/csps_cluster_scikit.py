...
db_time = 0
write_data = False
#themes = ['RR', 'MW', 'OP', 'LM', 'MT', 'MD', 'IF', 'RW', 'PB', 'LC']
themes = ['MW', 'OP', 'LM', 'MT', 'IF', 'RW', 'PB', 'LC']
...
# something to output data to a text file
def dump_all_data(yr, ftrs, data, mode="w"):
    fh = open("cluster_data.txt", mode)
    fh.writelines(str(yr))
    fh.writelines("\t")
    fh.writelines(ftrs)
    fh.writelines("\t")
    fh.writelines(data)
...
# finds intersection of two flat lists - useful in identifying which measures to remove
# as not all measures will be present at all times
def intersect(a, b):
...
def get_data(use_mysql):
    global db_time, feature_set, themes
#     organisation = ''
#     year = 2014
#     algorithm = 'KMeans'
    ...
    if (use_mysql):
        ...
        try:
            ...
            with connection.cursor() as cursor:
                ...
                theme_questions = " ".join(map(make_measure, themes))
                ...
                if (feature_set == 'questions'):
                    ...
                    sql_statement = sql_statement + measures + theme_questions + ");"      
                ...
                if (feature_set == 'ew_questions'):
                    ...
                    sql_statement = sql_statement + str(year) + " AND `headcount`.`year` = "
                    sql_statement = sql_statement + str(year) + " AND (`scores`.`measure` = 'EEI'"
                    sql_statement = sql_statement + e_measures + theme_questions + ");"
                ...
                if (feature_set == 'demographics'):
                    ...
                    sql_statement = "SELECT `org_demographics_ONS`.`organisation`, `org_demographics_ONS`.`year`, `org_demographics_ONS`.`headcount`, `org_demographics_ONS`.`acronym` as 'org', ROUND(((`male`-`female`)/100), 4) as 'gender_offset',  `ethnic_percentage`, `disabled_percentage`, ROUND(((`headcount_delta`/`headcount`)*100), 2) as 'headcount_delta', `scores`.`measure`, `scores`.`score` "
                    ...
                    sql_statement += str(year) + " AND `acronym` != 'ALL' AND (`scores`.`measure` = 'EEI'"
                    sql_statement += theme_questions + ");"
                    #print (sql_statement)
...
def do_clustering(csps_data):
    ...
    if (feature_set == 'demographics'):
        ...
        csps_data = pd.pivot_table(csps_data, values='score', index=['organisation', 'year', 'headcount', 'org', 'gender_offset',  'ethnic_percentage', 'disabled_percentage', 'headcount_delta'], columns=['measure'], aggfunc=np.sum)
...
def main():
    global year, feature_set
    if (write_data):
#         year = 2010
#         feature_set = 'ew_questions'
#         cluster_output = do_clustering(get_data(True))
#         print(cluster_output)
        years = range(2009, 2016)  # TODO - change back to 2016
        features = ['questions', 'themes', 'demographics', 'ew_questions']
        for yr in years:
            for feat_set in features:
                feature_set = feat_set
                year = yr
                #print( '*' * 80 )
                #print str(year) + ' ' + feature_set
                cluster_output = do_clustering(get_data(True))
                #print(cluster_output)
                
                # uncomment line below to save into a text file
                # dump_all_data(year, feature_set, cluster_output, "a")
    else:
        cluster_output = do_clustering(get_data(True))
        print cluster_output
